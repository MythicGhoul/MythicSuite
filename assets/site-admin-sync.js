...
// =========================================================
// SITE-WIDE ADMIN METADATA SYNCHRONISATION
// =========================================================
(() => {
  const pathname = window.location.pathname;

  const catalogueUrl = pathname.includes("/plugins/")
    ? "../../assets/plugins.json"
  ...
  const lifecycleName = lifecycle => {
    ...
    return names[String(lifecycle || "current").toLowerCase()] || "Current";
  };
  ...
  const hasPublicPlatform = plugin =>
    Boolean(plugin?.spigot?.id || plugin?.modrinth?.id);
  ...
  const updateInfoRow = (root, label, value) => {
    ...
    const row = [...root.querySelectorAll(".info-row")].find(candidate =>
      candidate.querySelector("small")?.textContent.trim().toLowerCase() ===
      label.toLowerCase()
    );
    ...
    const target = [...row.children].find(child => child.tagName !== "SMALL");
    setText(target, value);
  };

  const updatePrivateCardStats = (card, plugin) => {
    if (hasPublicPlatform(plugin)) return;

    const spans = card.querySelectorAll(".card-stats > span");
    if (!spans.length) return;

    const statusLabel = plugin.statusLabel || lifecycleName(plugin.lifecycle);

    if (spans[0]) {
      spans[0].textContent = "";
      const strong = document.createElement("b");
      strong.textContent = statusLabel;
      spans[0].append(strong);
    }

    if (spans[1]) {
      spans[1].textContent = "";
      const strong = document.createElement("b");
      strong.textContent = lifecycleName(plugin.lifecycle);
      spans[1].append(strong);
    }
  };
  ...
  const applyCardMetadata = (card, plugin) => {
    ...
    const status = card.querySelector(".status-chip");
    if (status) {
      setText(status, plugin.statusLabel || "Public Plugin");
      replaceStatusClass(status, plugin.lifecycle);
    }

    setText(card.querySelector(".card-category"), plugin.category);
    renderTags(card.querySelector(".tag-row"), plugin.tags || []);
    updatePrivateCardStats(card, plugin);
  };

  const updateDetailActions = plugin => {
    if (hasPublicPlatform(plugin)) return;

    const action = document.querySelector(".detail-actions .button");
    if (!action) return;

    const messages = {
      exclusive: "Private server project",
      development: "Currently in development",
      planned: "Direct download coming soon",
      legacy: "Legacy project",
      current: "No public download yet"
    };

    action.textContent =
      messages[String(plugin.lifecycle || "current").toLowerCase()] ||
      "No public download yet";
  };

  const updateNonPublicMetrics = plugin => {
    if (hasPublicPlatform(plugin)) return;

    const versionStrong = document.querySelector(
      "[data-live-release-version]"
    );
    if (versionStrong) {
      versionStrong.textContent =
        plugin.statusLabel || lifecycleName(plugin.lifecycle);

      const versionLabel = versionStrong.closest(".metric")?.querySelector("span");
      if (versionLabel) versionLabel.textContent = "Release status";
    }

    const updateStrong = document.querySelector(
      "[data-live-release-date]"
    );
    if (updateStrong) {
      updateStrong.textContent = "No public release";

      const updateLabel = updateStrong.closest(".metric")?.querySelector("span");
      if (updateLabel) updateLabel.textContent = "Public availability";
    }

    const releasePanel = document.querySelector("[data-live-plugin-release]");
  ...
  const applyDetailMetadata = plugin => {
    ...
    updateDetailActions(plugin);
    updateNonPublicMetrics(plugin);
    document.body.dataset.lifecycle = plugin.lifecycle || "current";
  };

  const applyGeneralPluginElement = (element, plugin) => {
    if (element.matches(".plugin-card")) {
      applyCardMetadata(element, plugin);
      return;
    }

    const status = element.querySelector(".status-chip");
    if (status) {
      setText(status, plugin.statusLabel || "Public Plugin");
      replaceStatusClass(status, plugin.lifecycle);
    }
    ...
    const category =
      element.querySelector(".card-category") ||
      element.querySelector(".studio-pill");

    if (category && plugin.category) setText(category, plugin.category);
    renderTags(element.querySelector(".tag-row"), plugin.tags || []);
  };

  async function synchroniseSite() {
    try {
      const response = await fetch(
        `${catalogueUrl}?sitewideSync=${Date.now()}`,
        { cache: "no-store" }
      );
      ...
      document.querySelectorAll("[data-plugin-id]").forEach(element => {
        const plugin = byId.get(element.dataset.pluginId);
        if (plugin) applyGeneralPluginElement(element, plugin);
      });

      const currentId = document.body.dataset.pluginId;
      if (currentId && byId.has(currentId)) {
        applyDetailMetadata(byId.get(currentId));
      }

      const activeFilter = document.querySelector(".filter-button.active");
      if (activeFilter) activeFilter.click();

      document.documentElement.dataset.metadataSynced = "true";
    } catch (_) {
      // Existing HTML remains as the fallback.
    }
  }

  if (document.readyState === "loading") {
    document.addEventListener("DOMContentLoaded", synchroniseSite, {
      once: true
    });
  } else {
    synchroniseSite();
